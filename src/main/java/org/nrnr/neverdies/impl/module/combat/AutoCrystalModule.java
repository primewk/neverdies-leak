...
public class AutoCrystalModule extends RotationModule {
    ...
    Config<Boolean> damageConfig = new BooleanConfig("Damage", "Renders the damage of the crystals", true);
    Config<Boolean> fadeConfig = new BooleanConfig("Fade", "Fade Renders", false);
    Config<Integer> fadeTimeConfig = new NumberConfig<>("Fade-Time", "Timer for the fade", 0, 5, 1000, () -> fadeConfig.getValue());
    Config<Boolean> newRenderConfig = new BooleanConfig("NewRender", "New Renders ", false);
    Config<Boolean> breakDebugConfig = new BooleanConfig("Debug", "Debugs break ms in data", false, () -> renderConfig.getValue());
    //
    Config<Boolean> disableDeathConfig = new BooleanConfig("Death", "Disables during disconnect/death", true);
    //
        ...
        if (renderConfig.getValue()) {
            for (Map.Entry<BlockPos, Animation> set : fadeList.entrySet()) {
                ...
                if (damageConfig.getValue()) {
                    ...
                    if (renderPos != null) {
                        RenderManager.post(() -> RenderManager.renderSign(event.getMatrices(), format.format(this.placeCrystal.getDamage()), this.renderPos.toCenterPos()));
                    }
                }
            }
